...
# 故事文本
scenes = [
    {'text': '从前，有三只小猪离开了妈妈，要自己盖房子住。'},
    {'text': '猪大哥很懒，用稻草盖了一座房子。'},
    {'text': '猪二哥也不勤快，用木头盖了一座房子。'},
    {'text': '猪小弟最勤劳，用砖头盖了一座坚固的房子。'},
    {'text': '一天，大灰狼来了！它吹倒了稻草房。'},
    {'text': '又吹倒了木头房！'},
    {'text': '但是砖头房子吹不倒！'},
    {'text': '三只小猪安全了！这个故事告诉我们：勤劳才能成功！'},
]

def create_scene_image(scene_index):
    """创建一个场景的图片"""
...
def main():
    ...
    for i, scene in enumerate(scenes):
        ...
        for dx in [-2, 2]:
            for dy in [-2, 2]:
                draw.text((x+dx, y+dy), scene['text'], font=font, fill=COLORS['black'])
        draw.text((x, y), scene['text'], font=font, fill=COLORS['white'])
        
        txt_path = f"{output_dir}/text_{i}.png"
        txt_img.save(txt_path)
        
        # 6. 创建文字片段
        txt_clip = ImageClip(txt_path).with_duration(audio_duration)
