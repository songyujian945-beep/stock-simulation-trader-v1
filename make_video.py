...
def create_text_clip(text, duration):
    ...
    txt_clip = txt_clip.set_position(('center', HEIGHT - 300))
...
def main():
    ...
    for i, scene in enumerate(scenes):
        print(f"📹 处理场景 {i+1}/{len(scenes)}: {scene['text'][:20]}...")
        ...
        audio_duration = audio_clip.duration + 0.5  # 稍微延长一点
        
        # 4. 创建视频片段
        video_clip = ImageClip(img_path).set_duration(audio_duration)
    ...
    print(f"⏱️  时长: {sum([c.duration for c in clips]):.1f} 秒")
